    ...
    <div style={styles.header}>
      <div style={styles.headerInner}>
        ...
        {view==='home' ? <div style={{display:'flex',alignItems:'center',gap:8}}>
          ...
          : <button style={styles.headerBack} onClick={onHome}>← Jobs</button>}
      </div>
    </div>
  );
}
  ...
  return (
    <div style={styles.screen}>
      ...
      {tab==='crew'&&<>
        {crew.map((m,i)=>(
          <Card key={m.id}>
            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12}}>
              <div style={{display:'flex',flexDirection:'column',gap:2}}><button style={styles.reorderBtn} onClick={()=>moveCrew(m.id,-1)} disabled={i===0}>▲</button><button style={styles.reorderBtn} onClick={()=>moveCrew(m.id,1)} disabled={i===crew.length-1}>▼</button></div>
        ...
        <div style={styles.hint}>Burdened rate = base wage × burden multiplier. OT at 1.5× after 8 hrs/day.</div>
      </>}
      ...
      {tab==='equipment'&&<>
        ...
        {trucks.map((tr,i)=>(
          <Card key={tr.id}>
            <div style={{display:'flex',alignItems:'center',gap:8}}>
              <div style={{display:'flex',flexDirection:'column',gap:2}}><button style={styles.reorderBtn} onClick={()=>{const a=[...trucks];if(i>0){[a[i],a[i-1]]=[a[i-1],a[i]];onUpdateTrucks(a);}}} disabled={i===0}>▲</button><button style={styles.reorderBtn} onClick={()=>{const a=[...trucks];if(i<a.length-1){[a[i],a[i+1]]=[a[i+1],a[i]];onUpdateTrucks(a);}}} disabled={i===trucks.length-1}>▼</button></div>
          ...
          <Card key={tr.id}>
            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
              <div style={{display:'flex',flexDirection:'column',gap:2}}>
                ...
                <button style={styles.reorderBtn} onClick={()=>{const a=[...trailers];if(i<a.length-1){[a[i],a[i+1]]=[a[i+1],a[i]];onUpdateTrailers(a);}}} disabled={i===trailers.length-1}>▼</button>
              </div>
        ...
        <div style={styles.hint}>Day rate = annual depreciation ÷ deployment days/year.</div>
      ...
      {tab==='stock'&&<>
        {stockItems.map((item,i)=>(
          <Card key={item.id}>
            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
              <div style={{display:'flex',flexDirection:'column',gap:2}}><button style={styles.reorderBtn} onClick={()=>moveStock(item.id,-1)} disabled={i===0}>▲</button><button style={styles.reorderBtn} onClick={()=>moveStock(item.id,1)} disabled={i===stockItems.length-1}>▼</button></div>
...
const styles={
  ...
  header:{background:COLORS.primaryDark,padding:'16px 18px',position:'sticky',top:0,zIndex:10},
